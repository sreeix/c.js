                ...
                return Promise.each(ephemeralPaths, function (pathObj) {
                    ...
                    return zkLib.exists(pathObj.path).then(function (pathStat) {
                        if(pathStat) {
                            ...
                            return zkLib.ensurePath(pathObj.path).then(_.partial(setDataIfNeeded, pathObj.path, pathObj.data));
                        }
                        return Promise.resolve();
                    });
                });
    ...
    return {
        create: function (path, data, cb) {
            return zkLib.ensurePath(path)
                ...
                .then(returning({path: path, close: closeNotifications})).tap(console.log)
                .nodeify(cb);
        }
    };
};
