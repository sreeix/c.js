...
module.exports = function(client) {
        ...
        client.on('state', function (state) {
            if(state === zookeeper.State.SYNC_CONNECTED){
                return Promise.each(ephemeralPaths, function (pathObj) {
                    return zkLib.exists(pathObj.path).get(0).then(function (pathStat) {
                        if(pathStat) {
    ...
    var addNotificationPath = function (pathInfo) {
        if(!_.contains(ephemeralPaths, pathInfo.path)) {
    ...
    return {
        ...
        create: function (path, data, mode, cb) {
            // path is mandatory and so is cb. rest could be defaulted
            if(arguments.length === 2 && _.isFunction(data)) {
                cb = data;
                mode = zookeeper.CreateMode.EPHEMERAL;
                data = null;
            }
            if(arguments.length === 3 && _.isFunction(mode)) {
                cb = mode;
                mode = zookeeper.CreateMode.EPHEMERAL;
            }
            var x = {path: path, close: function () {
                if(x.active) {
                    x.active = false;
            ...
            console.log("making ", pathInfo);
            return makeNode(pathInfo).get(0).then(function (path) {
                x.path = pathInfo.path = path; // needed in case of sequential ephemeral nodes
                return x;
            }).then(watchPath)
