...
module.exports = function(client) {
    ...
    var makeNode = function (pathInfo) {
        ...
        return zkLib.ensurePath(zkPath.parent(path)).then(function (parent) {
            ...
            var p = data ? zkLib.create(path, new Buffer(data), mode): zkLib.create(path, mode);
            return p.catch(function handleNodeExistsAndSetDataIfNeeded(err) {
                // this is a bit messy. Iff node_exists we will not call it failure, but try and set data, but we are **NOT** setting the mode. This is because once we create it node mode is irrelevant
                if(err && err.name === 'NODE_EXISTS') {
                    console.log("Sombody created the nodes before me. But that's ok.");
                    if(data) {
                        return zkLib.setData(path, data).return(path);
                    }
                    return Promise.resolve(path);
                }
                throw err; // if it is a real failure. Let app deal with it.
            });
        });
    };
