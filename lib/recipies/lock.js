            ...
            var l = {
                ...
                lock: function (timeout, cb) {
                    ...
                    return zkLib.ensurePath(path)
                        .then(function (path) {
                            var lockPath = zkPath.join(path, 'lock-'+ zkClient.getSessionId().toString('hex')+ "-");
                            return zkLib.create(lockPath, new Buffer('lock'), zookeeper.CreateMode.EPHEMERAL_SEQUENTIAL);
                        })
