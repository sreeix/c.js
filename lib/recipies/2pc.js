...
/// It is important to be aware of the recovery mechanisms because the sites need to implement that.
// *


// For using classic 2 phase commit following options is used.
...
module.exports = function  (zkClient) {
    ...
    function trackConsensus(options) {
        ...
        if(_.isNumber(options.quorum) ){
            console.log("quorum", options.quorum);
        ...
        var resolveIfConsensus = function () {
            // TODO use groupby to avoid iterating sites multiple times. (It may not be that big a deal though)
        ...
        var watchFn =function name(event) {
            ...
            if(type === zookeeper.Event.NODE_DELETED){
                ...
                // 1) The node previously agreed to commit and wrote it to it's transaction log -> In this case we keep the status as COMMIT(when the node comes back up it will)
//                updateFn(path, null);
            }
