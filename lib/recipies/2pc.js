...
module.exports = function  (zkClient) {
    ...
    var consensus = {
        ...
        init: function (options) {
            ...
            consensus.fsm.init({
                ...
                onAborting: function (context) {
                    ...
                    consensus.resolve('ABORT');
                },
                onPreparing: function (context) {
                    context.preparing = {};
                },
                preparingTransitionFn: function (event, context) {
                    console.log("Transitition fucntion in preparing", event, context);
                    ...
                    if(type === zookeeper.Event.NODE_DATA_CHANGED){
                        zkLib.getData(event.getPath()).get(0).then(_.bind(consensus.update, this, path, context));
                    }
                },
        ...
        watcher: function (event) {
            consensus.fsm.sendEvent(event);
        },
        update: function (path, context, data) {
    ...
    function setupWaitForConsensus(path, options) {
        consensus.init(options);
        return zkLib.watchAllChildren(path, consensus.watcher).each(function (child) {
            ...
            //     .then(_.partial(consensus.update, child));
