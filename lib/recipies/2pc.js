...
module.exports = function  (zkClient) {
    ...
    var consensus = {
        init: function (options) {
            ...
            consensus.promise = new Promise(function () {
            ...
            consensus.fsm = fsm({
                ...
                abortingTransitionFn: function (event, context) {
                    ...
                    if(type === zookeeper.Event.NODE_DATA_CHANGED){
                        zkLib.getData(event.getPath()).get(0).then(_.bind(consensus.updatePhase2Abort, this, path, context));
                    }
                },
    ...
    function watchSites(path, consensus) {
        return zkLib.watchAllChildren(path, consensus.watcher)
            ...
            .catch(function  (err) {
                ...
                return {promise: Promise.resolve('ABORT'), phase2Promise: Promise.resolve('ABORTED')};
            });
    }
                        ...
                        .then(function commitTransaction(state) {
                            ...
                            return watchSites(transactionPath, consensus).then(function () {
                                _.each(sites, function send(site) {
                                    return state === 'COMMIT'?  site.commit(command) : site.abort(command);
                                });
        ...
        site: function  (transactionPath, id, intentFn, commitFn, abortFn) {
            var site = {
                ...
                commit: function  (command) { // ideally this should be the transactionid
                    var p =  Promise.resolve('COMMITTED');
                    if(commitFn && _.isFunction(commitFn)){
                        p = Promise.promisify(commitFn)(command).then(returning('COMMITTED')).catch(returning("ABORTED"));
                    }
                    return p.then(_.bind(zkLib.setData, zkLib, site.selfPath()));
                },
                abort: function  (command) {
                    var p =  Promise.resolve('ABORTED');
                    if(abortFn && _.isFunction(abortFn)){
                        p = Promise.promisify(abortFn)(command).then(returning('ABORTED')).catch(returning("ABORTED"));// the last bit is probably wrong, but it does not matter anyways.
                    }
                    return p.then(_.bind(zkLib.setData, zkLib, site.selfPath()));
                }
            };
