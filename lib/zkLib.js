...
module.exports = function (client, opts) {
    ...
    var l = {
        ...
        rmr: retryFn(function removeRecursive(path) {
            ...
            return l.exists(path).then(function (stat) {
                if(stat) {
                    return client.getChildrenAsync(path).then(function (children) {
         ...
         **/
        getAndWatchNodeData: function (path, options, watcher, x) {
            var args = [path];
            if(options.data) { // apply the datawatcher only if it is expected to watch the data. Othewise just set the current data value and move on, it does not update.
                args.push(function  dataWatcher(event) {
                    ...
                    x.children = _.reject(x.children, function (c) {
                        ...
                        l.getAndWatchNodeData(path, options, watcher, x);
                    }
                    watcher(event);
                });
            }

            return client.getDataAsync.apply(client, args)
                .then(function (data) {
        ...
        getAndWatchNodeChildren: function (path, options, watcher, x) {
            var depthReducedOptions = _.clone(options); // should not clone if it is infinite
            depthReducedOptions.depth--;
            return client.getChildrenAsync(path, function childrenwatcher (event){
                ...
                if(existingChild) {
                    ...
                    return l.addSelfAndChildWatcher(zkPath.join(path, child), depthReducedOptions, watcher);
                }
        ...
        addSelfAndChildWatcher: function (path, options, onWatch, x) {
            ...
            return l.getAndWatchNodeData(path, options, onWatch, root).then(function () {
                if(options.depth > 0) {
                    console.log("Watching for children", root.path, options.depth);
